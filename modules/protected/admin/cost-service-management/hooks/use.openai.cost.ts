...
import {
    fetchOpenAiCostChartClient,
    fetchOpenAiCostSummaryClient,
    triggerOpenAiCostSyncClient,
} from "@/services/client/openai.cost.service";
import { DEFAULT_PRESET_RANGE } from "../constants/cost.service.constants";
import {
    FormattedOpenAiChartPoint,
...
export function useOpenAiCost() {
    const [presetRange, setPresetRange] =
        useState<OpenAiCostPresetRange>(DEFAULT_PRESET_RANGE);
    const [customGranularity, setCustomGranularity] =
        useState<OpenAiCostGranularity>("Daily");
    ...
    const chartParams = useMemo<OpenAiCostChartParams>(() => {
        switch (presetRange) {
            case "last30Days":
                return {
                    timeframe: "Custom",
    ...
    const currentGranularity: OpenAiCostGranularity =
        chartParams.granularity || "Monthly";

    // Fetch Summary (Month to Date cost - OpenAI API)
    const {
        ...
        queryKey: queryKeys.openAiCost.summary,
        queryFn: fetchOpenAiCostSummaryClient,
    ...
    // Fetch Chart Data (Preset / Custom - OpenAI API)
    const {
        data: chartResponse,
        ...
        queryKey: queryKeys.openAiCost.chart(
            chartParams as Record<string, unknown>,
        ),
        queryFn: () => fetchOpenAiCostChartClient(chartParams),
        staleTime: 1000 * 60 * 5,
        retry: (failureCount, error) => {
            if (
                error instanceof ApiError &&
    ...
    const handleSync = async () => {
        try {
            setIsSyncing(true);
            setSyncMessage(null);
            await triggerOpenAiCostSyncClient();
            await refetchAll();
