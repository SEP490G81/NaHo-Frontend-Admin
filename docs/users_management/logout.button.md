...
- Sửa lại cho tôi file `layouts/protected-header/components/protected.header.tsx` như sau:

+ Xóa `layouts/sidebar/components/account.menu.tsx` và chuyển nút `logout` sang header và style cho nút đó giống với 2
  components là `NotificationButton` hoặc `SettingsButton` nhưng sẽ có border và text màu đỏ.
+ Phần `layouts/sidebar/components/user.avatar.tsx` sẽ hiện thêm role của người dùng đang đăng nhập hiện tại và email
  (không để vào trong account menu nữa vì sẽ xóa)
